...
- **③ 出発前スケジュール** — 渡航先＋渡航予定日＋初回に接種を受けられる日（起点。未入力なら今日）→ 推奨ワクチンの接種日を逆算。横型カレンダー（タイムライン）と表を併記。各ワクチンに CDC 推奨度（全渡航者／ほとんど／一部・条件付き／検討）をバッジ表示し、渡航先の対象ワクチンを推奨度別に一覧。迅速化スケジュール（＝接種間隔を短縮した承認済みの接種法）の説明と対応ワクチン一覧、間に合わない場合の警告、生ワクチン間隔・マラリア内服開始時期の注意、出発前の受診回数の目安。エンジンは `schedule.js`（ブラウザ／Node 共用）
