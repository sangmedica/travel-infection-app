...
- **③ 出発前スケジュール** — 渡航先を選ぶと推奨ワクチンが CDC 推奨度別（全渡航者／ほとんど／一部・条件付き／検討）にチェックリスト表示され（初期状態は全選択・全選択/全解除ボタンあり）、**接種を希望するワクチンだけ**を対象に、渡航予定日＋初回に接種を受けられる日（起点。未入力なら今日）から接種日を逆算。横型カレンダー（タイムライン）と表を併記。迅速化スケジュール（＝接種間隔を短縮した承認済みの接種法）の説明と対応ワクチン一覧、間に合わない場合の警告、生ワクチン間隔・マラリア内服開始時期の注意、出発前の受診回数の目安。エンジンは `schedule.js`（ブラウザ／Node 共用）
